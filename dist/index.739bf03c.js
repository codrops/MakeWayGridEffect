...
class SpreadGrid {
	 ...
	 */ expand(item) {
        ...
        for (let otherItem1 of filteredArray){
            const { x , y  } = this.expanded === -1 ? {
                x: 0,
                y: 0
            } : (0, _utils.getTranslationDistance)(otherItem1.DOM.el, item.DOM.el, this.options.spread, this.options.maxDistance);
            const delay = 0 //this.expanded === -1 ? 0 : map(getDistance(otherItem.DOM.el, item.DOM.el), 0, 1500, 0, 0.2);
            ;
            const zIndex = Math.round((0, _utils.map)((0, _utils.getDistance)(otherItem1.DOM.el, item.DOM.el), 0, 100000, 998, 1));
            const rotationInterval = this.options.maxRotation ? Math.max(Math.round((0, _utils.map)((0, _utils.getDistance)(otherItem1.DOM.el, item.DOM.el), 0, 500, this.options.maxRotation, 0)), 0) : 0;
            this.tl.set(otherItem1.DOM.el, {
                zIndex: this.expanded === -1 ? 1 : zIndex,
                delay: delay
            }, this.expanded === -1 ? "end" : "start").to(otherItem1.DOM.el, {
                x: x,
                y: y,
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["cVgJb","ebWYT"], "ebWYT", "parcelRequiree603")
